import { UserAddFormController, UserCreateFormModel } from "./UserCreateForm.types";
 ...
 */
const getDefaultValues = (initialData?: UserCreateFormModel) => {
    const defaultValues = {
        email: "",
        name: "",
        password: "",
        phoneNumber: "",
        // give roledefaultValue as UserRoleEnum.User
 ...
 */
export const useUserCreateFormController = (onSubmit?: () => void): UserAddFormController => {
    ...
    const submit = useCallback((data: UserCreateFormModel) => // Create a submit callback to send the form data to the backend.
        add(data).then(() => {
    ...
    const {
        ...
        setValue,
        formState: { errors }
    } = useForm<UserCreateFormModel>({ // Use the useForm hook to get callbacks and variables to work with the form.
