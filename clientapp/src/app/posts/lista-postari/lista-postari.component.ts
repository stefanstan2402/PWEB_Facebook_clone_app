...
import { BusinessAPIGetPage2OptionalParams, Post, PostDTO } from 'src/api/business/lib/models';
import { ListaBaseComponent } from 'src/app/shared/lista.base.component';

@Component({
  selector: 'app-lista-postari',
  templateUrl: './lista-postari.component.html',
  styleUrls: ['./lista-postari.component.css']
})
export class ListaPostariComponent extends ListaBaseComponent implements OnInit {

  public override source: PostDTO[];
  public override rows: PostDTO[] = [];
  public paramsForPaginatedResponse: BusinessAPIGetPage2OptionalParams = {
    search: null,
    page: 1,
    pageSize: 10
  };
  ...
  public override async incarcaDate() {
    ...
    try {
      ...
      let response = await this.getData(api => api.getPage2(this.paramsForPaginatedResponse));
      if (response.errorMessage != null) {
        ...
        if (this.source.length == 0) {
          ...
          await this.getData(api => api.getPage3(this.paramsForPaginatedResponse)).then(response => {
  ...
  public override vizualizare(item) {
    this.navigateToRelative(`${item?.id}/details`, this.activatedRoute);
  }

  public override modifica(item) {
    let idUser = sessionStorage.getItem('$AuthIdUtilizator$');
    if (idUser != item.user.id) {
      this.notificationService.showError('You can only edit your own posts!');
      return;
    }
    this.navigateToRelative(`${item?.id}`, this.activatedRoute);
  }

  public override async sterge(item: PostDTO) {
    try {
      // extract the iduser from session storage
      let idUser = sessionStorage.getItem('$AuthIdUtilizator$');
      if (!idUser) {
      ...
      if (idUser != item.user.id) {
        this.notificationService.showError('You can only delete your own posts!');
        return;
      }
      await this.postData(api => api.delete4(item.id, idUser)).then(response => {
        if (response.errorMessage != null) {
          this.notificationService.showError(response.errorMessage.message);
        } else {
