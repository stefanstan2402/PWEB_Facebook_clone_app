...
import { BusinessAPIGetPage3OptionalParams, UserDTO, UserDTOPagedResponse } from 'src/api/business/lib/models';
...
export class ListaUsersComponent extends ListaBaseComponent implements OnInit {

  public override source: UserDTO[];
  public override rows: UserDTO[] = [];
  public paramsForPaginatedResponse: BusinessAPIGetPage3OptionalParams = {
    search: null,
    page: 1,
    pageSize: 10
  };
  ...
  public override async incarcaDate() {
    ...
    try {
      ...
      let response = await this.getData(api => api.getPage3(this.paramsForPaginatedResponse));
  ...
  public override async sterge(item: UserDTO) {
    try {
      await this.postData(api => api.delete5(item.id));
