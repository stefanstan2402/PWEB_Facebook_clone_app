...
public class CommentController: ControllerBase // Here we use the AuthorizedController as the base class because it derives ControllerBase and also has useful methods to retrieve user information.
{
    ...
    public async Task<ActionResult<RequestResponse<CommentDTO>>> GetById([FromRoute] Guid id) // The FromRoute attribute will bind the id from the route to this parameter.
    {
        ...
        return this.FromServiceResponse(await CommentService.GetComment(id));
    }
    ...
    {
        return this.FromServiceResponse(await CommentService.GetCommentCount(idPost));
    }
    ...
    {
        return this.FromServiceResponse(await CommentService.GetCommentsForPost(idPost));
    }
    ...
    public async Task<ActionResult<RequestResponse>> Add([FromBody] CommentAddDTO Comment)
    {
        try 
        {
            return this.FromServiceResponse(await CommentService.AddComment(Comment));
        }
        catch (Exception e)
        {
    ...
    {
        return this.FromServiceResponse(await CommentService.UpdateComment(Comment));
    }
    ...
    {
        return this.FromServiceResponse(await CommentService.DeleteComment(id, idUser, idPost));
    }
}
